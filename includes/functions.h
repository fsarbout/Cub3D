...
void    	cast_rays(t_dt *dt);
float    	normalize_angle(float angle);
void 		print_line (int x1, int y1);
void    	ray_casting(t_dt *dt , int i);
void    	fix_cast_angle();
void    	move_playeer();
...
void        sprite_calc( t_dt *t , int i);
// void        lst_add_back(t_list **head, t_sp spr);
float       normalize_sp_angle(float angle);
 void        sort_list(t_sp sp);
// void        clear_list(t_list **lst);
void        render_sprite(t_dt *dt);
// int         check_exists(t_list *lst, float x, float y);
void        draw_sprite( t_dt *dt);
void        render_one_sprite(t_dt *dt);
