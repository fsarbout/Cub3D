...
/*   Updated: 2021/01/17 12:03:16 by fsarbout         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	treat_sprite(char **element_data)
{
	...
	g_txt.sprite_txt = (int*)mlx_get_data_addr(g_txt.sp_txt, &g_dt.bpp
...
void	sprite_calc(t_list **list)
{
	...
	sp.angle = normalize_angle(sp.angle);
	...
	sp.height_e = sp.height_e > (g_dt.rsltn_h)
		? (g_dt.rsltn_h) - 1 : sp.height_e;
...
void	render_one_sprite(t_dt *dt, t_list **list)
{
	...
	{
		g_dt.x_of = ((64 * (start - disp->sp.width_s)
					* TILE / disp->sp.height) / 64);
		y = disp->sp.height_s;
...
void	print_it(int y, int start, t_list **list)
{
	...
	g_dt.y_of = d * ((TILE * 1.0) / disp->sp.height);
	g_dt.color = (int)(g_txt.sprite_txt[(int)(64 * g_dt.y_of + g_dt.x_of)]);
	if (g_dt.color != 0)
